...
Plans:
- [ ] 01-01-PLAN.md — SSH into VM, install system packages, clone repo, create python3.10 venv, install all requirements.txt dependencies
- [ ] 01-02-PLAN.md — Transfer .env to VM via SCP, validate settings with get_settings(), confirm Supabase connectivity with test_connection
...
**Success Criteria** (what must be TRUE):
  ...
  3. After killing the Gunicorn process (e.g. `kill <pid>`), systemd restarts it within the configured restart delay
  4. `curl http://<VM_IP>:5000/health` from an agent machine succeeds while the service is running under systemd
  5. UFW allows inbound traffic on port 5000 (`sudo ufw status` lists the rule) and `curl` from an agent machine confirms port is reachable

Plans:
- [ ] 04-01-PLAN.md — Write and activate systemd unit file; configure UFW with port 5000 and 2214 rules
- [ ] 04-02-PLAN.md — Human verification: reboot persistence, crash recovery, external /health reachability
...
**Plans**: TBD
**Success Criteria** (what must be TRUE):
  1. `AGGREGATOR_API_URL` in each agent machine's `.env` points to `http://<VM_IP>:5000/api/ingest` (not a PythonAnywhere URL)
...
| 4. Systemd Service | 0/2 | In Progress (Task 1 done; SSH lockout — needs console fix) | - |
