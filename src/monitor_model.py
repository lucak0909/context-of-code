...
class DataCollector:
    ...
    def get_network_metrics(self, use_cache: bool = True) -> NetworkMetrics:
        ...
        with BlockTimer():
            ...
            download_speed, method = self._measure_download_speed(speedtest_client)
    ...
    def _simple_download_test(self) -> float:
        try:
            ...
            test_urls = [
                "http://ftp.heanet.ie/mirrors/ubuntu/ls-lR.gz", # ~15MB file from Irish National Research Network
                "http://speedtest.tele2.net/10MB.zip",           # European speedtest generic
    ...
    def _measure_packet_loss(self) -> Tuple[float, float]:
        ...
        if not hosts: return 0.0, 0.0

        losses, pings = [], []
        for host in hosts:
            loss, ping = self._measure_packet_loss_host(host, packets)
            losses.append(loss)
            if ping > 0: pings.append(ping)
        ...
        try:
            ...
            try:
                ...
                for _ in range(packets):
                    ...
                    if res is None or res is False: loss_count += 1
                    else: latencies.append(res * 1000)
    ...
    def _subprocess_ping(self, host: str, packets: int) -> Tuple[float, float]:
        try:
            ...
            if is_win: args.insert(3, "-w"); args.insert(4, "2000")
            
            proc = subprocess.run(args, capture_output=True, text=True, timeout=(packets*2)+5)
            out = (proc.stdout + "\n" + proc.stderr).lower()
            ...
            if loss is not None: return loss, avg
            return 0.0, avg
        except Exception:
            return 100.0, 0.0
